...
namespace HdrHistogram.Utilities
{
    ...
    public sealed class ByteBuffer
    {
        ...
        {
            var shortValue = BinaryPrimitives.ReadInt16BigEndian(new ReadOnlySpan<byte>(_internalBuffer, Position, 2));
        ...
        public int GetInt()
        {
            var value = BinaryPrimitives.ReadInt32BigEndian(new ReadOnlySpan<byte>(_internalBuffer, Position, 4));
            Position += 4;
            return value;
        }
        ...
        public long GetLong()
        {
            var value = BinaryPrimitives.ReadInt64BigEndian(new ReadOnlySpan<byte>(_internalBuffer, Position, 8));
            Position += 8;
            return value;
        }
        ...
        {
#if NETSTANDARD2_0
            var bits = BinaryPrimitives.ReadInt64BigEndian(new ReadOnlySpan<byte>(_internalBuffer, Position, 8));
            var value = BitConverter.Int64BitsToDouble(bits);
#else
            var value = BinaryPrimitives.ReadDoubleBigEndian(new ReadOnlySpan<byte>(_internalBuffer, Position, 8));
#endif
            Position += 8;
            return value;
        }
        ...
        {
            BinaryPrimitives.WriteInt32BigEndian(new Span<byte>(_internalBuffer, Position, 4), value);
            Position += 4;
        }
        ...
        {
            BinaryPrimitives.WriteInt32BigEndian(new Span<byte>(_internalBuffer, index, 4), value);
        ...
        {
            BinaryPrimitives.WriteInt64BigEndian(new Span<byte>(_internalBuffer, Position, 8), value);
            Position += 8;
        }
        ...
        {
#if NETSTANDARD2_0
            BinaryPrimitives.WriteInt64BigEndian(new Span<byte>(_internalBuffer, Position, 8), BitConverter.DoubleToInt64Bits(value));
#else
            BinaryPrimitives.WriteDoubleBigEndian(new Span<byte>(_internalBuffer, Position, 8), value);
#endif
            Position += 8;
        }
